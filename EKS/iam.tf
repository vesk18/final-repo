...
data "tls_certificate" "cert" {
  url = aws_eks_cluster.vesk18.identity[0].oidc[0].issuer
}
...
module "oidc-provider-data" {
  ...
  version    = "0.0.3"
  issuer_url = aws_eks_cluster.vesk18.identity[0].oidc[0].issuer
}

# For nodes
resource "aws_iam_role" "danit-node" {
  name = "vesk18-eks-node"
...
POLICY
  tags = merge(
    var.tags,
    { Name = "vesk18-eks-node-role" }
  )
}

resource "aws_iam_policy" "secrets_policy" {
  name        = "vesk18-GetSecrets"
  path        = "/"
